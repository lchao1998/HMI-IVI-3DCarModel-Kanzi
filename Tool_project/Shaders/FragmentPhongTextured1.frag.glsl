...
varying mediump vec2 smokecoord;
#endif

#if KANZI_SHADER_USE_BASECOLOR_TEXTURE
uniform lowp sampler2D tex;
uniform lowp sampler2D smoke;
#endif
...
void main()
{
    ...
    float fresnel = pow(abs(dot(vViewDirection,vNormal)),1.0);
    float opacity = abs(dot(texture2D(tex, texcoord).rgb,abs(vNormal)));
    float mask = smoothstep(0.0,0.5,abs(dot(texture2D(smoke, smokecoord).rgb,abs(vNormal))));
    
    lowp vec4 baseColor = vec4(0.9,1.0,0.9,(fresnel*opacity*mask)*0.7);
    //lowp vec4 baseColor = vec4(0.7,1.0,0.8,0.0);
#else
